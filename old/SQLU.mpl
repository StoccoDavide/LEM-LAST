# Signature utilities
module Sig()
    export ModuleApply, UnVeil, Veil, LastUsed, UnVeilTable, forgetVeil, ShowVeil;
    local SIG,  NextLabel;
...
local A, n, k, i, ii, j, m, L, U, mltip, l, r, kp, p, temp, normalize, row, flag;
A := copy(xA);
n := LinearAlgebra[RowDimension](A):
m := LinearAlgebra[ColumnDimension](A);
if m<>n then error "The input matrix should be square!" end if;
L := Matrix(LinearAlgebra[IdentityMatrix](n),shape=triangular[lower]);  # L is lower matrix with 1's as diagonal entries
...
for kp from 1 to m while row <= n do             # loop over columns
    # We find the pivot element among the entries A[r[kp],kp], A[r[kp+1],kp], .... A[r[n],kp]
    ...
    if Strategy_Zero(A[r[p],kp], Prim, Q)=0 then
        ...
        # Now do Guass elimination steps to get new A and also keep L information in new A
        # Packing everything into the A matrix during the computation of the factors.
        # The reason is so that this code can be ported to restricted memory environments,
        # or equally, applied to very large matrices in a large memory environment.
        for i from row+1 to n do
